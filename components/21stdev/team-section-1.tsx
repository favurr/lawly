...
interface SocialLink {
  icon: React.ComponentType<any> | unknown;
  href: string;
}
  ...
  (
    ...
    return (
      <section
        ref={ref}
        className={cn(
          "relative w-full mx-auto overflow-hidden bg-background py-12 md:py-24 lg:py-32",
          className
        )}
        {...props}
      >
        <div className="container grid mx-auto items-center justify-center gap-8 px-4 text-center md:px-6">
          ...
          {socialLinksMain && socialLinksMain.length > 0 && (
            <div className="relative z-10 flex w-full items-center justify-center gap-4 py-4 md:justify-center">
              {socialLinksMain.map((link, index) => (
                ...
                >
                  <HugeiconsIcon icon={link.icon as never} size={24} />
                </a>
              ))}
                  ...
                  <div className="relative z-10 mt-4 flex gap-3 opacity-0 transition-opacity duration-300 ease-in-out group-hover:opacity-100">
                    {member.socialLinks.map((link, linkIndex) => (
                      <a
                        ...
                        className="text-muted-foreground hover:text-primary transition-colors"
                      >
                        <HugeiconsIcon icon={link.icon as never} size={20} />
                      </a>
                    ))}
                  </div>
                )}
